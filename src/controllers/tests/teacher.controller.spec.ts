...
//
  it('should call registerStudent in TeacherService with the correct data', async () => {
    const mockDto = { teacher: 'teacherken@gmail.com', students: ['studentjon@gmail.com'] };
...
//
  it('should call getCommonStudents in TeacherService with the correct data', async () => {
    ...
    const mockResponse = { students: ['studentjon@gmail.com', 'studentbob@gmail.com'] };
    jest.spyOn(teacherService, 'getCommonStudents').mockResolvedValue(mockResponse as unknown as GetCommonStudentsResponseDto);

    const result = await teacherController.getCommonStudents(mockEmails);

    expect(teacherService.getCommonStudents).toHaveBeenCalledWith(mockEmails);
...
//
  it('should call suspendStudent in TeacherService with the correct data', async () => {
    ...
    await teacherController.suspendStudent(mockDto as unknown as SuspendStudentRequestDto);
...
//
  it('should call retrieveForNotifications in TeacherService with the correct data', async () => {
    ...
    jest.spyOn(teacherService, 'retrieveForNotifications').mockResolvedValue(mockResponse);

    const result = await teacherController.retrieveForNotifications(mockDto);

    expect(teacherService.retrieveForNotifications).toHaveBeenCalledWith(mockDto);
    expect(result).toEqual(mockResponse);
  });
